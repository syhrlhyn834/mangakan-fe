...
export default {
  ...
  methods: {
    async fetchMangas(page) {
      try {
        const response = await this.$axios.get(`/api/web/groups/${this.$route.params.slug}?page=${page}`);
    ...
    changePage(page) {
      this.$router.push({ name: 'group-slug', params: { slug: this.$route.params.slug }, query: { pageManga: page } });
