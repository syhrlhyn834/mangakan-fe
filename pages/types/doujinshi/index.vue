  ...
  <div class="bg-[#191b1d] min-h-screen flex flex-col">
    <div class="px-4 mb-2 flex-grow">
      <div class="bg-[#ff6740] text-white p-1 md:p-1.5 rounded mb-2 flex justify-between items-center">
        <h2 class="text-xl font-bold"><i class="fas fa-folder-open mr-1"></i> Type: Doujinshi Upload Terbaru</h2>
      </div>
...
export default {
  ...
  stopImageRotation() {
    ...
    async fetchMangas(page) {
      try {
        const response = await this.$axios.get(`/api/web/doujinHome?page=${page}`);
