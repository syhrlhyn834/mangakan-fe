      ...
      <div class="px-4 mb-2 flex-grow">
        ...
        <div class="bg-[#ff6740] text-white p-1 md:p-1.5 rounded mb-2 flex items-center">
          <h2 class="text-lg font-semibold">
            <i class="fas fa-folder-open mr-1"></i> characters
          </h2>
        </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            >
              <div class="flex-1 bg-gray-800 p-2">
                <nuxt-link :to="{name: 'character-slug', params: {slug: genre.slug}}" class="text-lg text-white font-semibold truncate">
                  {{ genre.name }}
                </nuxt-link>
              </div>
...
export default {
  head() {
  return {
    title: 'Daftar Character',
  };
},
  async asyncData({ $axios }) {
    try {
      const response = await $axios.get('/api/web/characters');
