<template>
  <div>
    <h1 class="text-2xl font-semibold mb-4">Author Manga</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="author in filteredauthors" :key="author.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-author-edit-id', params: { id: author.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deleteauthor(author.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-author-create' })"
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/authors?q=${search}&page=${page}`);
      return {
        filteredauthors: response.data.data,
  ...
  methods: {
    async fetchauthors() {
      try {
        const response = await this.$axios.$get(`/api/admin/authors?q=${this.search}`);
        this.filteredauthors = response.data.data;
    ...
    async deleteauthor(id) {
      try {
        const confirmed = await this.$swal.fire({
          title: 'Are you sure?',
          text: 'You are about to delete this author.',
          icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/authors/${id}`);
          await this.fetchauthors(); // Refresh data after deletion
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/authors?q=${this.search}&page=${page}`);
        this.filteredauthors = response.data.data;
        this.pagination = response.data;
