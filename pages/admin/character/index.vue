<template>
  <div>
    <h1 class="text-2xl font-semibold mb-4">character Manga</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="character in filteredcharacters" :key="character.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-character-edit-id', params: { id: character.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deletecharacter(character.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-character-create' })"
  ...
  >
    ...
    <span class="sr-only">Add character</span>
  </button>
</div>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/characters?q=${search}&page=${page}`);
      return {
        filteredcharacters: response.data.data,
  ...
  methods: {
    async fetchcharacters() {
      try {
        const response = await this.$axios.$get(`/api/admin/characters?q=${this.search}`);
    ...
    async deletecharacter(id) {
      try {
        const confirmed = await this.$swal.fire({
          title: 'Are you sure?',
          text: 'You are about to delete this character.',
          icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/characters/${id}`);
          await this.fetchcharacters(); // Refresh data after deletion
          await this.$swal.fire({
            title: 'Deleted!',
            text: 'The character has been deleted.',
            icon: 'success',
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/characters?q=${this.search}&page=${page}`);
        this.filteredcharacters = response.data.data;
        this.pagination = response.data;
