<template>
  <div>
    <h1 class="text-2xl font-semibold mb-4">manga Manga</h1>
      ...
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="manga in filteredmangas" :key="manga.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
              {{ manga.characters.map(character => character.name).join(', ') }}
            </td>
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
              {{ manga.status }}
            </td>
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-manga-edit-id', params: { id: manga.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deletemanga(manga.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-manga-create' })"
  ...
  >
    ...
    <span class="sr-only">Add manga</span>
  </button>
</div>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/mangas?q=${search}&page=${page}`);
      return {
        filteredmangas: response.data.data,
  ...
  methods: {
    async fetchmangas() {
      try {
        const response = await this.$axios.$get(`/api/admin/mangas?q=${this.search}`);
        this.filteredmangas = response.data.data;
    ...
    async deletemanga(id) {
      try {
        const confirmed = await this.$swal.fire({
          title: 'Are you sure?',
          text: 'You are about to delete this manga.',
          icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/mangas/${id}`);
          await this.fetchmangas(); // Refresh data after deletion
          await this.$swal.fire({
            title: 'Deleted!',
            text: 'The manga has been deleted.',
            icon: 'success',
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/mangas?q=${this.search}&page=${page}`);
        this.filteredmangas = response.data.data;
        this.pagination = response.data;
