<template>
  <div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="series in filteredseries" :key="series.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-series-edit-id', params: { id: series.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deleteseries(series.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div
      data-dial-init
      class="fixed bottom-6 right-6 series"
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-series-create' })"
  ...
  >
    ...
    <span class="sr-only">Add series</span>
  </button>
</div>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/series?q=${search}&page=${page}`);
      return {
        filteredseries: response.data.data,
  ...
  methods: {
    async fetchseries() {
      try {
        const response = await this.$axios.$get(`/api/admin/series?q=${this.search}`);
        this.filteredseries = response.data.data;
    ...
    async deleteseries(id) {
      try {
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/series/${id}`);
          await this.fetchseries(); // Refresh data after deletion
          await this.$swal.fire({
            title: 'Deleted!',
            text: 'The series has been deleted.',
            icon: 'success',
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/series?q=${this.search}&page=${page}`);
        this.filteredseries = response.data.data;
        this.pagination = response.data;
