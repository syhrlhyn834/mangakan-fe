<template>
  <div>
    <h1 class="text-2xl font-semibold mb-4">Genre Manga</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="genre in filteredgenres" :key="genre.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-genre-edit-id', params: { id: genre.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deletegenre(genre.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-genre-create' })"
  ...
  >
    ...
    <span class="sr-only">Add genre</span>
  </button>
</div>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/genres?q=${search}&page=${page}`);
      return {
        filteredgenres: response.data.data,
  ...
  methods: {
    async fetchgenres() {
      try {
        const response = await this.$axios.$get(`/api/admin/genres?q=${this.search}`);
        this.filteredgenres = response.data.data;
    ...
    async deletegenre(id) {
      try {
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/genres/${id}`);
          await this.fetchgenres(); // Refresh data after deletion
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/genres?q=${this.search}&page=${page}`);
        this.filteredgenres = response.data.data;
        this.pagination = response.data;
