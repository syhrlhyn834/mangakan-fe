...
export default {
  ...
  methods: {
    async fetchMangas(page) {
      try {
        const response = await this.$axios.get(`/api/web/series/${this.$route.params.slug}?page=${page}`);
    ...
    changePage(page) {
      this.$router.push({ name: 'series-slug', params: { slug: this.$route.params.slug }, query: { pageManga: page } });
